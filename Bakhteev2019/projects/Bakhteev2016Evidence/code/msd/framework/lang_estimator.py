...
class LangevinEstimator(AbstractEstimator):
	def __init__(self,  param_length, param_scale,  X, Y, ind, builder, **kwargs):
		...
		for i in xrange(0, self.multistart):
			...
			self._debug = grad
			updates.append((params, params - self.lr *  grad +rvn*T.sqrt(2*self.lr) ))
			models.append(model)
	...
	def update(self, ind):
		...
		for echange in self.entropy_change:
			r0 = np.random.randn(self.param_length).astype(np.float32)
			r1 = echange(r0, ind)
			r2 = echange(r1.astype(np.float32), ind)						
	...
	def evidence(self,ind):
		like = -np.array(self.neg_likelihood(ind))
		prior = np.array(self.prior())
		entropy = self.entropy
		evidence =  prior +  like + entropy
		
		return np.mean(evidence), (np.mean(prior), np.mean(like), np.mean(entropy), evidence)
