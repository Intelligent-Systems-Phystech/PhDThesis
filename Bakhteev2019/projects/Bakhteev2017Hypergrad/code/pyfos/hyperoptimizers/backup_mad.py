...
def drmad_optimize(trainig_criterion, model_constructor, param_optimizer,  trial_num , batch_size, train_iteration_num, X_data, Y_data, hyperparams, learning_rate_param, 
    lr=0.0, lr_for_learning_rate=0.0, limits=None, lr_limits=None, verbose=0, use_hessian=False):
    ...
    param_tensors = [T.vector() for _ in xrange(k)]
    ...
    vgp = theano.function(indices+param_tensors, valid_grad_params, givens=givens)
    vgh = theano.function(indices+param_tensors, valid_grad_hyper, givens=givens, on_unused_input='ignore')

    cgp = theano.function(indices+param_tensors, cost_grad_params, givens=givens)
    ...
    _new_cgph = theano.function(indices+param_tensors+vectors, _new_cgph,  givens= givens) 

    W0 = [np.array(m.params.eval()) for m in training_procedure.models]
    if use_hessian:
        vectors = [T.vector() for _ in xrange(k)]

        cgpp = T.Rop(cost_grad_params, all_params, vectors)
        cgpp = theano.function(indices+param_tensors+vectors, cgpp,  givens= givens) 
    for trial in xrange(trial_num):
	 ...
	 #print 'training', time.time()  - time_s
        ...
        dw = np.array(vgp(*(training_procedure.validation_indices+W1))  )
       
        dhyp = np.array(vgh(*(training_procedure.validation_indices+W1))    )
        if optimize_learning_rate:
            dl = 0
        #print  dhyp
        ...
        for i in xrange(train_iteration_num):
            ...
            if optimize_learning_rate:
                ...
                dls = []
                for vt_, dw_ in zip(vt, dw):
                    dls.append(np.dot(dw_.T, vt_))
                dl += np.mean(dls)
              
            dv = (dw*learning_rate_param.eval())
            
            if use_hessian:
                args = batch+W+dv.tolist()
            ...
            print 100*(time.time() - time_s)
            #print '2'
            #_dv = dv.flatten()
            #print '3'
            dhyp = dhyp + _concat_cgph#np.dot(_dv,_concat_cgph) #pure
            #dhyp = dhyp + np.dot(np.mean(dv, axis=0), np.mean(cgph( *(batch+W)), axis=0)) #with mean approximation


            if verbose>=0 and (verbose==0 or i%verbose==0):
                
                print 'reverse-iteration {0}, gradients:{1} time:{2}'.format(str(i), str(dhyp), str(time.time() - time_s))
                print 'max abs grad', np.max(abs(dhyp))
		time_s = time.time()
		if optimize_learning_rate:
