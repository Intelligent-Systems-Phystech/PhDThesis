...
def build_var_feedforward(structure,  params=None, init_sigmas = None, nonlinearity = T.tanh, use_softmax = False, log_alphas = theano.shared(np.array([0.0]).astype(theano.config.floatX)), dataset_size=1, megabatch_size=50,  bias=True, param_pool_size=1, log_var_sigmas=None):
    if bias:
        bias_coef =1                                                                                                                                                                                                                                                                
    else:
        bias_coef = 0
    ...
    def build_likelihood(X, Y):
        random_pool_selector = T.mod(T.arange(X.shape[0]),param_pool_size)#srng.choice(size=(X.shape[0],), a=param_pool_size)
        output = X
        mult_factor = X.shape[0]/param_pool_size+1
        ...
        for i in xrange(len(_structure)-1):
                if i== len(structure)-2:
                    ...
                    nonlin = nonlinearity
                W = Ws_var[i][random_pool_selector]
                #W = T.tile(Ws_var[i], (mult_factor, 1, 1))[:X.shape[0]]#[random_pool_selector]
                if bias:
                    #b = T.tile(bs_var[i], (mult_factor, 1))[:X.shape[0]]
                    b = bs_var[i][random_pool_selector]
                    output = nonlin(T.batched_dot(output, W)  + b)
                else:
                    output = nonlin(T.batched_dot(output, W) )
        if use_softmax:
    ...
    def build_cost(X,Y):
        log_likelihood  = build_likelihood(X,Y)
...
if __name__=='__main__':
    ...
    """
    model = build_var_feedforward([1,2], nonlinearity=lambda x:x, dataset_size=3, param_pool_size=10, log_alphas=np.array([-10.0, -10.0]), bias=False,
use_softmax=True)
    model.params.set_value([1.0,2.0, -10.0, -10.0])
    predict =theano.function([X, Y], model.cost(X,Y))
    print predict([[1.0]], [1])
