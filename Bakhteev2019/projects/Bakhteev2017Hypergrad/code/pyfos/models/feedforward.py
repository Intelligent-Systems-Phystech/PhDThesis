...
def build_feedforward(structure,  params=None, init_sigmas = None, nonlinearity = T.tanh, use_softmax = False, log_alphas = theano.shared(np.array([0.0])), dataset_size=1, bias=True):
    ...
    if len(log_alphas.eval())==1:
        prior = gaus_prior(params, log_alphas=log_alphas, gaus_type='scalar')
    else:
        prior = gaus_prior(params, log_alphas=log_alphas, gaus_type='diagonal')
    ...
    def build_cost(X,Y):
        ...
        return log_likelihood*dataset_size/X.shape[0]+prior
    ...
    def build_validation(X,Y):
        ...
        if use_softmax:
            ...
            log_likelihood  = -0.5*T.sum((outputs[-1].T - Y)**2) - np.log(2*np.pi)*(X.shape[1]*X.shape[0]*1.0/2)            
        #print dataset_size
        return log_likelihood*dataset_size/X.shape[0]
